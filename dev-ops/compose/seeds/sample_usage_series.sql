...
INSERT INTO projects (id, originated_id, title, origination, project_pi_id, status, created_time) VALUES
  ('smpu-project', 'SMPU-REC-001', 'Aurora Genomics', 'TEST', 'smpu-user-pi', 'ACTIVE', NOW())
ON CONFLICT DO NOTHING;
...
INSERT INTO compute_allocations
  ...
  ('smpu-alloc-healthy', 'smpu-project', 'Aurora Mixed', 'ACTIVE', 'smpu-cluster', 2000000,
     CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '120 days')
ON CONFLICT DO NOTHING;

INSERT INTO compute_allocation_memberships
  (id, compute_allocation_id, user_id, start_time, end_time, membership_status) VALUES
  ('smpu-cam-ar-pi',   'smpu-alloc-atrisk',  'smpu-user-pi',   CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '15 days',  'ACTIVE'),
  ('smpu-cam-ar-mem1', 'smpu-alloc-atrisk',  'smpu-user-mem1', CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '15 days',  'ACTIVE'),
  ('smpu-cam-ar-mem2', 'smpu-alloc-atrisk',  'smpu-user-mem2', CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '15 days',  'ACTIVE'),
  ('smpu-cam-he-pi',   'smpu-alloc-healthy', 'smpu-user-pi',   CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '120 days', 'ACTIVE'),
  ('smpu-cam-he-mem1', 'smpu-alloc-healthy', 'smpu-user-mem1', CURRENT_DATE - INTERVAL '40 days', CURRENT_DATE + INTERVAL '120 days', 'ACTIVE')
ON CONFLICT DO NOTHING;
...
SELECT
  ...
  st.a,
  GREATEST(1, (st.b + st.s * ((d.n * 37 + 11) % 9)) / st.rd),
  st.b + st.s * ((d.n * 37 + 11) % 9),
  CURRENT_DATE - (34 - d.n) * INTERVAL '1 day' + INTERVAL '12 hours',
  st.u,
  CONCAT('job-', st.k, '-', d.n),
  st.r
