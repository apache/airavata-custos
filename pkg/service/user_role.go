...
// ensureSystemOrgTx returns the organization for users without an upstream one, creating it on first use.
...
// ensureAdminRoleTx creates the admin role on first use. Privileges are seeded only at creation.
