...
func TestGrantPrivilege_HappyPath(t *testing.T) {
	...
	grant, err := svc.GrantPrivilege(ctx(), target, models.PrivilegeAMIERead, granter, "ops view")
	if err != nil {
		t.Fatalf("GrantPrivilege: %v", err)
	}
	if grant.UserID != target || grant.Privilege != models.PrivilegeAMIERead {
		t.Errorf("grant payload mismatch: %+v", grant)
	}

	has, err := svc.HasPrivilege(ctx(), target, models.PrivilegeAMIERead)
	if err != nil || !has {
...
func TestGrantPrivilege_RejectsGranterWithoutMeta(t *testing.T) {
	...
	_, err := svc.GrantPrivilege(ctx(), target, models.PrivilegeAMIERead, granter, "")
...
func TestRevokePrivilege_HappyPath(t *testing.T) {
	...
	if err := svc.RevokePrivilege(ctx(), target, models.PrivilegeAMIERead, granter, "no longer needed"); err != nil {
...
func TestRevokePrivilege_NoActiveGrant_ReturnsNotFound(t *testing.T) {
	...
	target := seedUser(t, database, "target@example.edu")
	seedPrivilegeGrant(t, database, granter)
	err := svc.RevokePrivilege(ctx(), target, models.PrivilegeAMIERead, granter, "")
...
func TestHasPrivilege_ReturnsFalseForUngranted(t *testing.T) {
	...
	has, err := svc.HasPrivilege(ctx(), user, models.PrivilegeAMIERead)
	if err != nil {
...
func TestListUserPrivileges_ReturnsActiveOnly(t *testing.T) {
	...
	if len(rows) != 1 || rows[0].Privilege != models.PrivilegeAMIERead {
		t.Errorf("active set: got %v, want [amie:read]", rows)
	}
}
...
func TestListPrivilegeHolders(t *testing.T) {
	...
	seedPrivilegeGrant(t, database, granter)
	if _, err := svc.GrantPrivilege(ctx(), a, models.PrivilegeAMIERead, granter, ""); err != nil {
		t.Fatalf("grant a: %v", err)
	}
	if _, err := svc.GrantPrivilege(ctx(), b, models.PrivilegeAMIERead, granter, ""); err != nil {
		t.Fatalf("grant b: %v", err)
	}
	rows, err := svc.ListPrivilegeHolders(ctx(), models.PrivilegeAMIERead)
	if err != nil {
...
func TestPrivilegeCatalog(t *testing.T) {
	...
	if !seen[models.PrivilegeGrant] || !seen[models.PrivilegeAMIERead] {
...
func TestSchema_RejectsTwoActivePrivilegesOfSameKey(t *testing.T) {
	...
	if _, err := database.Exec(
		...
		uuid.NewString(), user, string(models.PrivilegeAMIERead),
	); err != nil {
