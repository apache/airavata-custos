...
func TestGrantPrivilege_HappyPath(t *testing.T) {
	...
	grant, err := svc.GrantPrivilege(ctx(), target, models.PrivilegeHPCRead, granter, "ops view")
	if err != nil {
		t.Fatalf("GrantPrivilege: %v", err)
	}
	if grant.UserID != target || grant.Privilege != models.PrivilegeHPCRead {
		t.Errorf("grant payload mismatch: %+v", grant)
	}

	has, err := svc.HasPrivilege(ctx(), target, models.PrivilegeHPCRead)
	if err != nil || !has {
...
func TestGrantPrivilege_RejectsGranterWithoutMeta(t *testing.T) {
	...
	_, err := svc.GrantPrivilege(ctx(), target, models.PrivilegeHPCRead, granter, "")
	if !errors.Is(err, ErrInvalidInput) {
		t.Errorf("granter w/o meta: got err=%v, want ErrInvalidInput", err)
	}
	if !strings.Contains(err.Error(), string(models.PrivilegeGrant)) {
...
func TestGrantPrivilege_DuplicateActiveReturnsAlreadyExists(t *testing.T) {
	...
	if _, err := svc.GrantPrivilege(ctx(), target, models.PrivilegeHPCRead, granter, ""); err != nil {
...
func TestRevokePrivilege_HappyPath(t *testing.T) {
	...
	if err := svc.RevokePrivilege(ctx(), target, models.PrivilegeHPCRead, granter, "no longer needed"); err != nil {
...
func TestRevokePrivilege_RejectsSelfRevokeOfMeta(t *testing.T) {
	...
	seedPrivilegeGrant(t, database, caller)

	err := svc.RevokePrivilege(ctx(), caller, models.PrivilegeGrant, caller, "self")
	if !errors.Is(err, ErrInvalidInput) {
...
func TestRevokePrivilege_NoActiveGrant_ReturnsNotFound(t *testing.T) {
	...
	target := seedUser(t, database, "target@example.edu")
	seedPrivilegeGrant(t, database, granter)
	err := svc.RevokePrivilege(ctx(), target, models.PrivilegeHPCRead, granter, "")
...
func TestHasPrivilege_ReturnsFalseForUngranted(t *testing.T) {
	...
	has, err := svc.HasPrivilege(ctx(), user, models.PrivilegeHPCRead)
	if err != nil {
...
func TestListUserPrivileges_ReturnsActiveOnly(t *testing.T) {
	...
	if err := svc.RevokePrivilege(ctx(), target, models.PrivilegeHPCRead, granter, ""); err != nil {
	...
	if len(rows) != 1 || rows[0].Privilege != models.PrivilegeHPCWrite {
		t.Errorf("active set: got %v, want [hpc:write]", rows)
	}
}
...
func TestListPrivilegeHolders(t *testing.T) {
	...
	seedPrivilegeGrant(t, database, granter)
	if _, err := svc.GrantPrivilege(ctx(), a, models.PrivilegeHPCRead, granter, ""); err != nil {
		t.Fatalf("grant a: %v", err)
	}
	if _, err := svc.GrantPrivilege(ctx(), b, models.PrivilegeHPCRead, granter, ""); err != nil {
		t.Fatalf("grant b: %v", err)
	}
	rows, err := svc.ListPrivilegeHolders(ctx(), models.PrivilegeHPCRead)
	if err != nil {
...
func TestPrivilegeCatalog(t *testing.T) {
	...
	if !seen[models.PrivilegeGrant] || !seen[models.PrivilegeHPCRead] {
...
func TestSchema_RejectsTwoActivePrivilegesOfSameKey(t *testing.T) {
	...
	if _, err := database.Exec(
		...
		uuid.NewString(), user, string(models.PrivilegeHPCRead),
	); err != nil {
