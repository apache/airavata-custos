...
// OnboardUser creates the same rows ingestion does, so the existing cluster
// user create event drives provisioning.
func (s *Service) OnboardUser(ctx context.Context, in OnboardUserInput) (*models.User, error) {
	...
	// Only holders of roles:manage may create admins.
	if admin {
	...
	// Resolved before the transaction so a taken name fails with a clear
	...
	if created.OrganizationID == "" {
		// Created ahead of the main transaction so the user check below can see
		// it. An unused system org is inert if the rest fails.
	...
	// Every row lands together or none does, so a half-made user cannot block
	// the retry on its own email.
	if err := s.inTx(ctx, func(tx *sql.Tx) error {
