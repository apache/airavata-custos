...
/**
 * Check a requested cluster username
 *
 * Returns an available suggested login and, when a candidate is given, whether it is well-formed and free on the event's cluster.
 */
export const getAccessRequestsUsername = <ThrowOnError extends boolean = false>(options: Options<GetAccessRequestsUsernameData, ThrowOnError>): RequestResult<GetAccessRequestsUsernameResponses, GetAccessRequestsUsernameErrors, ThrowOnError> => (options.client ?? client).get<GetAccessRequestsUsernameResponses, GetAccessRequestsUsernameErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
