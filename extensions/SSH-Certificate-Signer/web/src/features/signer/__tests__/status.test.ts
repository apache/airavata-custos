...
describe("getCertificateStatus", () => {
  ...
  it("returns not-yet-valid before valid_after", () => {
    expect(getCertificateStatus({ ...base, valid_after: 1_800_000_000 }, now)).toBe(
      "not-yet-valid",
    );
  });
...
describe("statusBadgeVariantFromCertificateStatus", () => {
  it("maps each status to a badge variant", () => {
    ...
    expect(statusBadgeVariantFromCertificateStatus("not-yet-valid")).toBe("pending");
