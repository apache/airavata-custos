...
// Calls stay inside the signer zone. Its BFF reads the shared portal session
// and forwards the administrator's bearer token to the signer service.
...
export async function revokeCertificate(serial: number, reason: string): Promise<RevokeResponse> {
  const raw = await apiFetch(`/admin/certificates/${serial}/revoke`, {
    method: "POST",
    body: { reason },
  });
