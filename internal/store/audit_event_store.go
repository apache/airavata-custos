...
const auditEventColumns = "id, event_type, event_time, entity_id, details, trace_id, span_id, parent_span_id"
...
func (s *mysqlAuditEventStore) Create(ctx context.Context, tx *sql.Tx, e *models.AuditEvent) error {
	_, err := tx.ExecContext(ctx,
		`INSERT INTO audit_events (id, event_type, event_time, entity_id, details, trace_id, span_id, parent_span_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		e.ID, e.EventType, e.EventTime, e.EntityID, e.Details, e.TraceID, e.SpanID, e.ParentSpanID)
	return err
}
