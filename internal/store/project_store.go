...
func (s *mysqlProjectStore) FindByID(ctx context.Context, id string) (*models.Project, error) {
	...
	err := s.db.GetContext(ctx, &p,
		`SELECT id, originated_id, title, origination, project_pi_id, status, created_time
...
func (s *mysqlProjectStore) UpdateStatus(ctx context.Context, tx *sql.Tx, id string, status models.AllocationStatus) error {
	_, err := tx.ExecContext(ctx,
