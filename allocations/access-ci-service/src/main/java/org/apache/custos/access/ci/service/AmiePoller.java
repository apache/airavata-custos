 ...
 */
@Component
@ConditionalOnProperty(name = "access.amie.poller.enabled", havingValue = "true", matchIfMissing = true)
public class AmiePoller {
    ...
    @Scheduled(fixedDelayString = "#{T(org.springframework.boot.convert.DurationStyle).detectAndParse('${access.amie.scheduler.poll-delay}').toMillis()}")
    @Transactional
