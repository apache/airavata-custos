...
// P2-1: consecutive constraint violations must retry with different
// uidNumbers. The retry loop bumps the min-uid floor above the last
// value it attempted so a stale LDAP scan cannot suggest the same
// number twice.
func TestEnsurePOSIXAccount_RetriesBumpUIDFloor(t *testing.T) {
	core := &fakeCore{user: sampleUser()}
	// The allocator scan (uidNumber=*) always returns the same entry
	// so max+1 alone would suggest the same uid every retry. The
	// FindPosixAccount lookup (uid=alovelace) stays empty so we hit
	// the fresh-allocation path.
	staleEntry := ldap.NewEntry("uid=stale,ou=people,dc=test", map[string][]string{
		"uidNumber": {"50000"},
	})
	conn := &fakeConn{
		perFilterResults: map[string]*ldap.SearchResult{
			"(&(objectClass=posixAccount)(uidNumber=*))": {Entries: []*ldap.Entry{staleEntry}},
		},
		pendingAddErrs: []error{
			&ldap.Error{ResultCode: ldap.LDAPResultConstraintViolation},
			&ldap.Error{ResultCode: ldap.LDAPResultConstraintViolation},
	...
	for i, r := range conn.addCalls {
		...
		u := attrs["uidNumber"][0]
		if seen[u] {
			t.Errorf("attempt %d reused uidNumber=%s; retries must bump the floor", i+1, u)
		}
		seen[u] = true
	}
	if len(seen) != 3 {
		t.Errorf("expected 3 distinct uidNumbers across retries, got %d: %v", len(seen), seen)
	}
}
