...
// Prefer AllocateAndAddPosixAccount for the common "assign a fresh
// uidNumber and write" path — that variant guarantees the allocate/write
// pair is atomic within the process.
...
// This is the direct-LDAP counterpart to COmanage's identifier-assignment
// plugin — LDAP itself is the identity registry.
//
// NOTE: this is v1 allocation. Two known limitations, both tracked as
// follow-up work:
//   - A deleted LDAP entry's uidNumber can be reused by a later Add,
//     which risks a new user inheriting file ownership stamped with
//     the old numeric uid on the cluster. A persistent monotonic
//     counter is the right long-term fix.
//   - Concurrent allocators across Custos instances rely on LDAP's
//     uidNumber uniqueness constraint (when configured) to catch
//     collisions; without it, silent duplicates are possible.
//
// See AllocateAndAddPosixAccount for the in-process atomicity guarantee.
...
// AllocateAndAddPosixAccount atomically allocates the next uidNumber
// AND writes the corresponding posixAccount entry under a single lock,
// closing the "another goroutine allocates the same number between my
// read and my write" race for in-process callers.
//
// The build callback lets the caller fill in name / home / shell etc.
// once the numeric id is known, without pushing that business logic
// into the client.
//
// Cross-process races are still possible in multi-instance deployments.
// LDAP's uidNumber uniqueness constraint (when configured) rejects the
// second Add and the caller retries — see IsConstraintViolation.
func (c *Client) AllocateAndAddPosixAccount(minUID int64, build func(uid int64) PosixAccount) (uid int64, dn string, err error) {
	c.mu.Lock()
	defer c.mu.Unlock()

	uid, err = c.allocateNextUIDLocked(minUID)
	if err != nil {
		return 0, "", err
	}

	acct := build(uid)
	dn, err = c.addPosixAccountLocked(acct)
	if err != nil {
		// Return the attempted uid so the caller can use it as a floor
		// on the next attempt.
		return uid, "", err
	}
	return uid, dn, nil
